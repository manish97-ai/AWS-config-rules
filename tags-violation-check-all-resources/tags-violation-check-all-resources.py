...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    if configuration_item["resourceType"] not in APPLICABLE_RESOURCES:
        violations = "The rule doesn't apply to resources of type " + configuration_item["resourceType"] + "."
        return "NOT_APPLICABLE"

    if configuration_item["configurationItemStatus"] == "ResourceDeleted":
        violations = "The configurationItem was deleted and therefore cannot be validated."
        return "NOT_APPLICABLE"

    current_tags = configuration_item["configuration"].get("tags")
    violations = find_violation(current_tags, valid_rule_parameters)
...
def lambda_handler(event, context):
    ...
    if not compliance_result:
        ...
        if configuration_item:
            evaluations.append(build_evaluation_from_config_item(configuration_item, compliance_result, annotation=violations))
        else:
            evaluations.append(
